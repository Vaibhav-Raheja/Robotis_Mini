...
def plot_waveform(y_values, t, title="Waveform"):
...
def sine_wave_input():
    ...
    global amplitude
    # time.sleep(0.2)
    num_samples = int((end_time - start_time) / control_period_)
    t = np.linspace(start_time, end_time, num_samples)
    y_values = amplitude * np.sin(2 * np.pi * freq_n * t)
    plot_waveform(y_values, t, "Sine Wave")  
    return y_values
...
def triangle_wave_input(): 
    ...
    num_samples = int((end_time - start_time) / control_period_)
    t = np.linspace(start_time, end_time, num_samples)
    y_values = amplitude * scipy.signal.sawtooth(2 * np.pi * freq_n * t, 0.5)
    plot_waveform(y_values, t, "Triangle Wave")
    return y_values
...
def execute_variable_foot_position(robot, z_foot_pos):
    ...
    for i in range(len(wave_signal)):  
        ...
        joint_values_right_foot = robot.ik_right_foot(robot.x_RF0 + x_offset, robot.y_RF0 + y, z_foot_pos, robot.roll_RF0, robot.pitch_RF0)
        joint_values_left_foot = robot.ik_left_foot(robot.x_LF0 + x_offset, robot.y_LF0 + y, z_foot_pos, robot.roll_LF0, robot.pitch_LF0)
        joint_values_right_hand = [0, 0, 0]
        joint_values_left_hand = [0, 0, 0]
        ...
        point = JointTrajectoryPoint()
        point.positions = joint_pos_values
        point.time_from_start = rospy.Duration(time_from_start+2)
        traj_msg.points.append(point)
